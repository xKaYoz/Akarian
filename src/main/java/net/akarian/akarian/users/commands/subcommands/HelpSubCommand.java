...
public class HelpSubCommand extends AkarianCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        for (AkarianCommand command : UserCommandManager.getInstance().getCommands().values()) {
            if (sender.hasPermission(command.getPermission())) commands.add(command);
        }

        if (commands.size() == 0) {
            Chat.sendRawMessage(sender, "&8&m----------------------------------------");
            Chat.sendRawMessage(sender, "&c&l  Akarian &f&lUser Help Menu");
        ...
        Chat.sendRawMessage(sender, "&8&m----------------------------------------");
        Chat.sendRawMessage(sender, "&c&l  Akarian &f&lUser Help Menu &7(" + (page) + "/" + (commands.size() % 10 == 0 ? commands.size() / 10 : (commands.size() / 10) + 1) + ")");
        Chat.sendRawMessage(sender, "");
